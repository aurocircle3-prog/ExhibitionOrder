...
app.get('/api/orders', resolveTenant, auth, async (req, res) => {
  ...
  if (req.user.role === 'client') {
    const user = await UserDB.findOne({ id: req.user.id });
    // Matches by phone OR email — a self-signed-up client's account is
    // keyed by email and may have no phone on file at all, but their
    // existing party record (from a past order) might still match by
    // email instead. Two targeted lookups, not a full collection scan —
    // this runs on every client order-list view.
    let party = user.phone ? await PartyDB.findOne({ tenantId: req.tenant.id, phone: user.phone }) : null;
    if (!party && user.email) party = await PartyDB.findOne({ tenantId: req.tenant.id, email: user.email.toLowerCase() });
